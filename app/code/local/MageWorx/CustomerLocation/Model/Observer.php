...
class MageWorx_CustomerLocation_Model_Observer
{
    ...
    {
        if(Mage::helper('geolocation')->isEnabledForOrders()){
            $_order = null;
            ...
            if(!is_null($_order)) {
                ...
                $geoIpObj = Mage::getModel('mwgeoip/geoip')->getLocation($ip);
                ...
                if ($geoIpObj->getCode()) {
                    ...
                    $block->getOrder()->setRemoteIp(Mage::helper('geolocation')->getGeoIpHtml($obj));
                }
            }
        }
        return $this;
    ...
    public function onlineCustomerLocation(Varien_Event_Observer $observer)
    {
        ...
        if (!Mage::helper('geolocation')->isEnabledForCustomers()) {
            return $this;
        }

        $block->addColumnAfter('geoip', array(
            'header'    => Mage::helper('mwgeoip')->__('IP Location'),
            'index'     => 'remote_addr',
            'align'     => 'left',
            'width'     => 200,
            'renderer'  => 'geolocation/adminhtml_customer_online_grid_renderer_geoip',
            'filter'    => false,
            'sortable'  => false,
        ), 'ip_address');
    }
