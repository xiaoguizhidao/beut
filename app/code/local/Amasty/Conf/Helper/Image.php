...
class Amasty_Conf_Helper_Image extends Mage_Core_Helper_Abstract
{
    ...
    private function getBaseUrl()
    {
		...
		$baseUrl = str_replace('media/', '', Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_MEDIA));
    ...
    public function imagePath($img='')
    {
		...
		$img = str_replace(Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_MEDIA), 'media/', $img);
        $img = trim(str_replace('/', DS, $img), DS);
        $this->img = BP . DS . $img;
		
        if((!file_exists($this->img) || !is_file($this->img)) && !empty($this->placeHolder) && $this->placeHolder != $image)
        {
