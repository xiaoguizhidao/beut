...
class Fishpig_Wordpress_Addon_AddThis_Block_Js extends Mage_Core_Block_Template
{
	/**
	...
	protected function _toHtml()
	{
		...
		if ($helper->isActiveOnPage()) {
			...
			return implode('', array(
				'<script type="text/javascript">var addthis_product = \'wpp-3.1\';',
				$this->_mergeArrayJson($config, $helper->getOption('addthis_config_json'), "var addthis_config = '%s';"),
				sprintf("var addthis_options = '%s';", $helper->getOption('addthis_options')),
				$this->_mergeArrayJson($share, $helper->getOption('addthis_share_json'), "if (typeof(addthis_share) == \"undefined\"){ addthis_share = '%s';}"),
				'</script>',
				sprintf('<script type="text/javascript" src="//s7.addthis.com/js/%s/addthis_widget.js#pubid=%s"></script>', $helper->getVersion(), $this->getPublisherId()),
			));
		}
