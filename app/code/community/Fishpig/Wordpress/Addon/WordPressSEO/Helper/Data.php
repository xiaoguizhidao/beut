...
class Fishpig_Wordpress_Addon_WordPressSEO_Helper_Data extends Fishpig_Wordpress_Helper_Plugin_Seo_Abstract
{
	 ...
	 * @return
	 */
	protected function _addOpenGraphTags($object, $type = 'post')
	{
		if (($head = Mage::getSingleton('core/layout')->getBlock('head')) !== false) {
			$tags = array(
				'site_name' => Mage::helper('wordpress')->getWpOption('blogname'),
				'type' => 'blog',
			...
			if ($object->getFeaturedImage()) {
				$image = $object->getFeaturedImage();
			...
			foreach($tags as $key => $value) {
				if (trim($value) !== '') {
					$tags[$key] = sprintf('<meta property="og:%s" content="%s" />', $key, addslashes(Mage::helper('core')->escapeHtml($value)));
				}
				else {
					unset($tags[$key]);
				}
