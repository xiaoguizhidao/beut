EWCoreHelpTooltip = {
	rewritePage: function() {
		$$(".ewcore-tooltip-label").each(function(e) {
		...
		$$("ewcore-tooltip-fieldset-label").each(function(e) {
	...
	toggleFieldTooltip: function(event) {
		var c = Event.findElement(event, '.ewcore-tooltip-label');
		var e = c.up().next('div');
		this.toggleTooltip(c, e, 0);
	},
	
	toggleFieldsetTooltip: function(event) {
		var c = Event.findElement(event, 'ewcore-tooltip-fieldset-label');
		var e = c.up().next('div');
		if (!e) {
			e = c.next('div');
		}
